...
"@
        ...
        # ═══════════════════════════════════════════════════════════════════
        # DESKTOP-ICONS: DEAKTIVIERT (verursacht Transparenz-Bug)
        # ═══════════════════════════════════════════════════════════════════
        # User muss Desktop-Icons manuell aktivieren:
        # Rechtsklick Desktop -> Ansicht -> Desktopsymbole anzeigen
        # Dann: Dieser PC, Papierkorb auswählen
        Write-SetupLog "Desktop-Icons: Manuelle Aktivierung erforderlich" -Level INFO
        
        if ($explorerNeedsRestart) {
            Write-SetupLog "`nStarte Explorer neu..." -Level INFO
            try {
                Stop-Process -Name explorer -Force -ErrorAction SilentlyContinue
                Start-Sleep -Seconds 2
                Start-Process explorer.exe
                Start-Sleep -Seconds 2
                Write-SetupLog "  [OK] Explorer neu gestartet" -Level SUCCESS
            }
            catch {
                Write-SetupLog "  [X] Explorer-Neustart: $_" -Level WARNING
            }
        }
    }
    catch {
